...
| Las páginas muestran "Cargando…" indefinidamente aunque la API responde `200` | En Angular 22 el ciclo de detección solo recompone las vistas marcadas como sucias y una propiedad plana mutada en un callback HTTP no marca la vista | Comprobar que el componente llame a `ChangeDetectorRef.markForCheck()` tras la respuesta, o migrar su estado a `signal()`. Corregido en `facturas`, `ventas`, `productos`, `clientes` y `login` |
| El login no muestra el mensaje de error y el botón sigue girando | Mismo caso anterior aplicado a `error` e `isLoading` de `login.component.ts` | Corregido con `markForCheck()` en el manejador `error`; ver "Detección de cambios" en el `README.md` |
| El catálogo del punto de venta queda cargando cuando falla la petición | `complete` de RxJS no se ejecuta después de un `error`, así que no se apagaba el indicador | Corregido llamando a `cerrarCargarCatalogo()` también en los manejadores `error` de `nueva-venta.component.ts` |
