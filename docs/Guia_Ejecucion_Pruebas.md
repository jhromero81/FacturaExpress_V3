...
| Las páginas muestran "Cargando…" indefinidamente aunque la API responde `200` | En Angular 22 el ciclo de detección solo recompone las vistas marcadas como sucias y una propiedad plana mutada en un callback HTTP no marca la vista | Declarar el estado con `signal()`: corregido en `facturas`, `ventas`, `productos`, `clientes` y `login`. `markForCheck()` también lo resolvía, pero se descartó al no ser necesario con señales |
| El login no muestra el mensaje de error y el botón sigue girando | Mismo caso anterior aplicado a `error` e `isLoading` de `login.component.ts` | Corregido declarando ambos como señales; ver "Detección de cambios" en el `README.md` |
