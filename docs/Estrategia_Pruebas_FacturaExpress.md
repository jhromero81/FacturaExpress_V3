...
La SPA compila con TypeScript en modo estricto (`strict` y `strictTemplates` activados en [`frontend/tsconfig.json`](../frontend/tsconfig.json)). Se eliminó `core/render.service.ts` (`ServicioRender`) y sus llamadas manuales a `ApplicationRef.tick()`: la detección de cambios la gestiona Zone.js con `provideZoneChangeDetection` y `withXhr`. En Angular 22 el ciclo solo recompone las vistas marcadas como sucias: las **Signals** se actualizan de forma implícita y los componentes que aún usan propiedades planas (`facturas`, `ventas`, `productos`, `clientes` y `login`) llaman a `ChangeDetectorRef.markForCheck()` tras cada respuesta HTTP; sin esa marca la vista quedaba en "Cargando…" con los datos ya en memoria. La búsqueda y la paginación de clientes, productos y facturas se resuelven en el servidor con los parámetros `q`, `estado`, `pagina` y `limite`, y los campos `total`/`totalPaginas` que devuelve la API; el punto de venta usa el IVA de cada producto y recarga el catálogo tras la venta. El repositorio incluye `frontend/.prettierrc.json`.
